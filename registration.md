...
- **0.2 Plain-language summary ★** — one paragraph, what the approach does (not how):
  We simulate synthetic survey respondents as LLM personas built from real Reddit users' own post and comment histories. 1,000 personas, sampled to match the parent megastudy's demographic recruitment quotas, are each shown the study's climate-trust survey interventions and asked to predict how that specific person would most likely have answered, given only what can be inferred about them from their own post history and infered demograpics.
...
- **B.2 Access & context mode** — API/web/local; API name + version; chat vs stateless; exact call dates:
  ...
  Independant run was started 28.8.2026 and ran for ~31h.
...
- **B.3 Configuration** — temperature, top-p/top-k, max tokens, penalties, stop sequences, seeds, reasoning effort, completions per item:
  Temperature 0.7, top_p 0.8 (Qwen3's recommended non-thinking settings), max_tokens 5000, no explicit penalties/stop sequences beyond the model's own. "Thinking" (chain-of-thought) mode explicitly disabled via `chat_template_kwargs.enable_thinking=false`. Base seed 20260818; per-(agent, condition[, step, attempt]) seeds derived deterministically from it. One completion per item per respondent (no repeated sampling/majority-vote).
...
- **B.4 Customization** — fine-tuning, RAG, prompt optimization, tool use, web search, agentic scaffolds (cross-ref H):
  ...
  Independent-block design (each of the 13 outcome blocks answered in its own call sharing only the treatment-exposure prefix).
...
- **B.5 Persistent memory** — across interactions? what persisted:
   ...
   Within one respondent × condition session, the model sees the user persona definition and intervention and is cached at this point, before answering each block of outcome questions (freshly, whith now knowledge of prior bock questions and answers).

- **B.6 Inference stack** — for local models: serving framework + version, quantization, hardware:
  vLLM v0.19.1, FP8 quantization, single GPU per pod (university Kubernetes cluster), `--max-model-len 16384`, prefix caching enabled (reuses the KV cache of the shared persona/treatment prompt prefix across a session's calls).
...
- **C.3 Prompt-design rationale** — brief rationale for the prompt design: why prompts were structured as they were, and the reasoning behind major design choices (recommended, not required):
  The "neutral annotator" framing (PoSSUM, Cerina (2025)) was chosen to reduce the model substituting its own views for the persona's. A brief explanation was requested per item for auditability, to make visible which features of a persona's actual data drove each answer.
...
- **E.2 Survey walk-through** — one item/call vs blocks vs whole survey; context carry-over; item/option ordering & randomization; scale display; attention/comprehension handling:
  **Primary entry (independent-block, `qwen27b_v1_indblocks`):** pretreatment (if enabled) and stimulus exposure answered first (KV cache at this point), then each of the 13 outcome blocks (one per survey page) is answered in its own separate call, each starting from the KV cached state, without any other block's answers, a deliberate anchoring/coupling mitigation. 
...
- **J.1 Design-space search †** — how the final pipeline was chosen: how many configurations tried, internal validation criterion, what data it ran against:
  The independent-block battery design (this submission's primary entry) was chosen over a sequential single-chain design (secondary-1 entry) based on validation against a held-out real-respondent task: the *Voelkel et al (2025)* climate-message megastudy (13,821 real respondents, real ground truth, via the *Ashokkumar et al (2026)* benchmark). 
...
- **K.3 Computational resources** — API-call counts, total tokens, cost, compute time:
  ...
  Runtime ~31h.
  Input Tokens: 1,396,233,357
  Output Tokens: 88,273,229
